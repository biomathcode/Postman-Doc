...
export const t = (text, color) => {
    return `<span style="color: ${color};">${text}</span>`
}

export const h = (text, type) => {
    switch (type) {
        case "h1":
            return `<h1>${text}</h1>`
        case "h2":
...
export const decisionBanner = text => {
    return `
<ac:adf-extension>
...
export const twoLayout = (first, second, type) => {
    return `
...
export const threeLayout = (first, second, third, type = "three_equal") => {
    return `
  <p/>
  <ac:layout-section ac:type="${type}" ac:breakout-mode="default">
      <ac:layout-cell>${first}</ac:layout-cell>
